...
**CRITICAL**: `mineru-open-api` is installed in conda environment `ai`. ALL commands MUST be prefixed with `conda run -n ai`. Never run bare `mineru-open-api` — it will fail with "command not found".

**Token**: Already configured via `MINERU_TOKEN` env var in conda env `ai`. Both flash-extract (free) and extract (precision) modes are ready to use. No auth step needed.

```bash
# ✅ CORRECT
conda run -n ai mineru-open-api flash-extract report.pdf
conda run -n ai mineru-open-api extract report.pdf -o ./out/
...
1. **Extract** — Use `conda run -n ai mineru-open-api` to convert the document to Markdown
...
conda run -n ai mineru-open-api flash-extract report.pdf --pages 1-20  -o ./out/
conda run -n ai mineru-open-api flash-extract report.pdf --pages 21-40 -o ./out/

# ✅ CORRECT — unique filenames per chunk
conda run -n ai mineru-open-api flash-extract report.pdf --pages 1-20  -o ./out/report_p1-20.md
conda run -n ai mineru-open-api flash-extract report.pdf --pages 21-40 -o ./out/report_p21-40.md
```
...
```bash
conda run -n ai mineru-open-api flash-extract report.pdf                               # to stdout (for immediate consumption)
conda run -n ai mineru-open-api flash-extract report.pdf -o ./output/                  # save to file
conda run -n ai mineru-open-api flash-extract report.pdf -o ./output/report_p1-10.md   # page range (explicit file path)
conda run -n ai mineru-open-api flash-extract report.pdf -o ./output/ --language en    # language hint
conda run -n ai mineru-open-api flash-extract https://example.com/paper.pdf            # URL input
```
...
Use when the user needs full-fidelity output: preserved images, accurate tables, LaTeX formulas, or non-Markdown formats. Requires a token via `conda run -n ai mineru-open-api auth`.

```bash
conda run -n ai mineru-open-api extract report.pdf                              # to stdout
conda run -n ai mineru-open-api extract report.pdf -o ./out/                    # save with all assets
conda run -n ai mineru-open-api extract report.pdf -o ./out/ -f md,docx         # multiple output formats
conda run -n ai mineru-open-api extract report.pdf -o ./out/report_p1-20.md --pages 1-20  # page range (explicit file path)
conda run -n ai mineru-open-api extract report.pdf -o ./out/ --ocr          # force OCR for scanned docs
conda run -n ai mineru-open-api extract *.pdf -o ./results/                 # batch processing
conda run -n ai mineru-open-api extract --list files.txt -o ./results/      # batch from file list
```
...
If the user hasn't authenticated yet, guide them to run `conda run -n ai mineru-open-api auth` first.
...
- **Debug API requests:** Add `-v` flag to see HTTP request/response details (e.g., `conda run -n ai mineru-open-api flash-extract report.pdf -v`)
...
- **Auth error on extract:** Run `conda run -n ai mineru-open-api auth` to set up your token
